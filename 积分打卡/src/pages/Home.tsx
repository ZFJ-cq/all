...
import { Zap, TrendingDown, Trophy, AlertCircle, Plus, Check, Flame } from 'lucide-react';
            ...
            <div className="space-y-2.5">
              {completed.map((task, idx) => (
                <TaskCard
                  ...
                  checked={true}
                  onToggle={(_e) => {
                    if (confirm('确认取消打卡？将扣除已获得的积分')) {
                      uncheckin(task.id, todayStr);
                    }
                  }}
                  delay={idx * 50}
                />
...
function TaskCard({ task, checked, onToggle, delay }: {
  task: { id: string; name: string; points: number; type: string; deadline?: string };
  checked: boolean; onToggle: (e: React.MouseEvent) => void; delay: number;
}) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        {/* Check Button */}
        <button
          onClick={canCheckin ? onToggle : () => { if (isExpired) alert('已超过截止时间，无法打卡'); }}
